...
- [c2patool](https://github.com/contentauth/c2patool) **v0.28.0** for Content Credentials (C2PA) inspection (CI and the API image pin it; on Windows unzip the release into `%LOCALAPPDATA%\Programs\c2patool-0.28.0\`, which the API finds first; older 0.9.x installs still work for development but cannot switch off the engine's own remote-manifest fetching) —
...
| `provenance` | no | C2PA manifests via c2patool (temp file, fixed args; capabilities probed once per process from `--help`; one retry only when the process cannot start); presence, signature validity (integrity codes only: a trust-list failure is never read as "altered"), stated signer, claim generator, actions, authors, validation codes grouped as success / informational / failure (`validation`, from `validation_results` on newer engines or synthesised from the flat list), `--info` facts; signed declarations (hash coverage, digital source types per action, training/mining permissions, CAWG identity presence, software agents), recursive ingredient tree with per-ingredient validation, manifest chain with signing-order check, `--tree` text; the engine runs with a shipped settings file that disables its own network access (no remote-manifest fetch, no OCSP); a remote manifest reference in XMP (`dcterms:provenance`) becomes the UNKNOWN record `provenance.remote` naming the host only; a second, separate trust run checks the signing certificate against the bundled official C2PA trust list (`provenance.trusted` VERIFIED when it chains to an anchor, `provenance.untrusted-signer` UNKNOWN when it does not, nothing when trust is off), with the list version recorded per analysis; absence is UNKNOWN; `GET /analysis/{id}/provenance` |
...
| `VERIXA_C2PA_REMOTE_MANIFEST_FETCH` | API | Let c2patool fetch manifests referenced by URL inside the asset (default false; refused in production; engines older than 0.28 cannot switch it off) |
| `VERIXA_C2PA_TRUST_MODE` | API | Signer trust evaluation: `bundled` (default; the official C2PA trust list shipped in `app/providers/provenance/trust/`), `custom` (your own local PEM files), or `off` |
| `VERIXA_C2PA_TRUST_ANCHORS_PATH` / `VERIXA_C2PA_ALLOWED_LIST_PATH` / `VERIXA_C2PA_TRUST_CONFIG_PATH` | API | Custom trust material (CA anchors, end-entity allow list, EKU config); local files only, never URLs |
